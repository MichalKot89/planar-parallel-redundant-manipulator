...
void Manipulator::WorkSpaceUsingForwardKinematics()
{
  ...
  FindWorkspaceOutline(Space, Space_Size_Y, Space_Size_X);
  
  for(unsigned int j=0; j<Space_Size_Y; j++)
    delete [] Space[j];
  delete [] Space;
}
...
void Manipulator::WorkSpaceUsingInverseKinematics()
{
  unsigned int Space_Size_X = ceil((SPACE_X_MAX - SPACE_X_MIN) / SPACE_GRID);
  unsigned int Space_Size_Y = ceil((SPACE_Y_MAX - SPACE_Y_MIN) / SPACE_GRID);
  double** Space = new double*[Space_Size_Y];
  ...
  for(unsigned int i=0; i<Space_Size_Y; i++) {
	...
	for(double l=MIN_FORCE_ANGLE; l<MAX_FORCE_ANGLE; l+=INVERSE_ANGLE_STEP) { 
	  ...
	  if(flague) {
	    /*if(i==6)
	    cout << endl << Q().Qa1().q()*180/M_PI << " " << Q().Qa2().q()*180/M_PI << " " 
	    << Q().Qb1().q()*180/M_PI << " " << Q().Qb2().q()*180/M_PI << endl;*/
	    Space[i][j]=1;
	    break;
	  }
	}
...
void Manipulator::FindWorkspaceOutline(double** Space, unsigned int Space_Size_Y, unsigned int Space_Size_X)
{
  ofstream outfile;
